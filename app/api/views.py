...
@jwt_required()
def logout():
    response = jsonify({"message": "Logout is successful"})
...
def post_todo():
    ...
    if not new_data.get("title") or not new_data.get("description"):
        return jsonify({"message": "There is no keys"}), 422 
...
def update_todo(id):
    ...
    try:
        db.session.commit()
        return jsonify({"message": "Todo had been updated"}), 204
...
def delete_todo(id):
      ...
      if not todo:
        return jsonify({"message": f"Todo with id = {id} is not found"}), 404
